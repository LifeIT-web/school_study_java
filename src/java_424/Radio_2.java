...
public class Radio_2 extends JFrame implements ActionListener{
	...
	public Radio_2() {
		...
		topPanel = new JPanel();
		JLabel label = new JLabel("어떤 크기의 커피를 주문하시겠어?");
		topPanel.add(label);
		...
		boxPanel.add(new JLabel("좋아하는 티를 선택 하쇼", JLabel.CENTER));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == small) {
			text.setText("small 크기가 선택됨");
		}
		if (obj == medium) {
			text.setText("medium 크기가 선택됨");
		}
		if (obj == large) {
			text.setText("large 크기가 선택됨");
		}
		if (obj == tea_1) {
			text.setText(tea_1.getText() +" 좋아해");
		}
		if (obj == tea_2) {
			text.setText(tea_2.getText() +" 좋아해");
		}
		if (obj == tea_3) {
			text.setText(tea_3.getText() +" 좋아해");
		}
	}
