...
public class ChatClient extends Frame implements ActionListener, Runnable{
	...
	final int port = 4561; //  다른 서버로 접속하려는 포트 번호
	
	CardLayout cl; // 카드 레이아웃 설정
	
	// 생성자
	public ChatClient(String title) {
		super(title); // 타이틀 바 제목
		...
		txt_server_ip = new TextField("192.168.0.92", 15); // 127.0.0.1 , localhost, 내 IP주소(IP사용 3가지 방법)
						// 다른 클라이언트가 내 PC에 접속하고 싶으면 내 PC의 IP주소를 알아야함.
		txt_name = new TextField("이지윤", 15);
